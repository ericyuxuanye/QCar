...
class Model:
    def __init__(self):
        ...
        self.qtable = [[[0.0, 0.0, 0.0] for i in range(15)] for j in range(15)]
        self.alpha = 0.1
        self.gamma = 0.9
        self.epsilon = 0.2
    ...
    def get_action(self, state1, state2):
        ...
        self.state = [(state1 - 200)//40, (state2 - 200)//40]
        # epsilon greedy
        if random.random() < self.epsilon:
            self.action = random.randint(0, 2)
            return self.action
        else:
            self.action = self.qtable[self.state[0]][self.state[1]].index(max(self.qtable[self.state[0]][self.state[1]]))
            return self.action
        
    def update(self, reward):
        if self.last_state is not None:
            self.qtable[self.last_state[0]][self.last_state[1]][self.last_action] += self.alpha * (reward + self.gamma * max(self.qtable[self.state[0]][self.state[1]]) - self.qtable[self.last_state[0]][self.last_state[1]][self.last_action])
    ...
    def save_qtable(self):
        with open("qtable.txt", "w") as f:
            for i in range(15):
                for j in range(15):
                    f.write(" ".join([str(x) for x in self.qtable[i][j]]) + "\n")